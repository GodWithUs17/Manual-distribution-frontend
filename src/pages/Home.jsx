...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      {/* Pass onOpenReceipt prop to Header */}
      <Header 
        searchQuery={searchQuery} 
        setSearchQuery={setSearchQuery} 
        onOpenReceipt={() => setIsModalOpen(true)} 
      />
      ...
      <main className="max-w-7xl mx-auto px-4 py-12">
        ...
        {/* Loading and Errors */}
        {loading && <p className="text-center py-10 font-medium text-gray-500">Loading manuals...</p>}
        {error && <p className="text-center text-red-500 py-10">{error}</p>}
        ...
        {!loading && !error && (
          <section>
            {filteredManuals.length > 0 ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
                {filteredManuals.map((manual, index) => (
                  <div key={manual.id} className="bg-white rounded-xl shadow-lg border border-[#E8D5B5] overflow-hidden flex flex-col group transition-all hover:shadow-2xl hover:-translate-y-2 duration-300">
                    
                    {/* Manual Card Top Section */}
                    <div className={`${index % 2 === 0 ? 'bg-[#002147]' : 'bg-emerald-600'} p-6 text-white min-h-[180px] relative flex gap-4`}>
                      <div className="flex-shrink-0 w-32 h-30 bg-white/20 rounded shadow-md overflow-hidden border border-white/30">
                        <img 
                          src={manual.imageURL || "/placeholder-manual.png"} 
